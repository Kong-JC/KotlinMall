...
abstract class BaseRecyclerViewAdapter<T, VH : RecyclerView.ViewHolder>(var mContext: Context) : RecyclerView.Adapter<VH>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
        mView = LayoutInflater.from(mContext).inflate(setView(), parent, false)
        return if (setViewHolder() == null) ViewHolder(mView) as VH else setViewHolder()
    }

    open fun setViewHolder(): VH = ViewHolder(mView) as VH
